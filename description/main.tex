...
\deadline{شنبه ؟ بهمن ۱۳۹۷، ساعت ۲۳:۵۵}
\graphicspath{{./img/}}
...
$ ./a.out courses.csv "Prog"
    ...
    پروندهٔ\LTRfootnote{file} برنامهٔ خود را با نام \lr{A1-SID.cpp} در صفحهٔ \lr{CECM} درس بارگذاری کنید که \lr{SID} شمارهٔ دانشجویی شماست؛ برای مثال اگر شمارهٔ دانشجویی شما ۸۱۰۱۹۷۹۹۹ باشد، نام پروندهٔ شما باید \lr{A1-810197999.cpp} باشد.
...
./a.out
    ...
    در این حالت برنامه ورودی‌اش را از ورودی استاندارد \lr{stdin} (خط فرمان) می‌خواند و خروجی را نیز در خروجی استاندارد \lr{stdout} (صفحهٔ خط فرمان) می‌نویسد.
...
./a.out < in.txt > out.txt
    ...
    برای این کار، ابتدا ورودی را از \lr{in.txt} به برنامه می‌دهیم و خروجی برنامه را در پرونده‌ای مانند \lr{out.txt} ذخیره می‌کنیم. سپس با دستور \lr{\lstinline[language=bash]{diff}} پروندهٔ \lr{out.txt} را با \lr{sol.txt} مقایسه می‌کنیم.
    \begin{latin}%
    \begin{Verbatim}[fontsize=\small]
g++ helloworld.cpp -std=c++11
./a.out < in.txt > out.txt
diff out.txt sol.txt
    ...
    اگر پرونده‌ها یکسان باشند، دستور \lr{\lstinline[language=bash]{diff}} هیچ خروجی‌ای تولید نمی‌کند. در غیر این صورت، تفاوت‌های دو پرونده را نشان می‌دهد.

    هر بخش از خروجی این دستور با شمارهٔ خطوط آغاز می‌شود: شمارهٔ خطوط در پروندهٔ قدیمی (سمت چپ)، یکی از حروف \lr{\texttt{d}}، \lr{\texttt{a}} یا \lr{\texttt{c}} و شمارهٔ خطوط در پروندهٔ جدید (سمت راست). حرف میان شمارهٔ خطوط نوع تغییرات را نشان می‌دهد:
    ...
    \end{minipage}%
    \begin{minipage}[t]{0.28\textwidth}
    \lstinputlisting[xleftmargin=10mm,frame=none,language=diff,basicstyle=\scriptsize\ttfamily\color{Cyan},label={diff}]{diff-res.txt}
    \end{minipage}
    \end{latin}    

    \section[آرگومان‌های خط فرمان]{آرگومان‌های خط فرمان\label{sec:commandline-args}\LTRfootnote{command line arguments}{\hfill\normalfont\footnotesize در نگارش این بخش از منبع~\cite{learncpp-commandlineargs} کمک گرفته شده است.}}
...
int main(int argc, char *argv[])
    ...
    دو آرگومان تابع \lr{\lstinline{main}} را برای می‌توان دسترسی به آرگومان‌های خط فرمان استفاده کرد:
    \begin{description}[leftmargin=5em,style=nextline,font=\labelitemi\quad\bfseries]
        ...
        \item[\lr{\texttt{argv}}] آرایه‌ای از رشته‌های مدل زبان \lr{C}؛ آرگومان‌های خط فرمان داده شده به برنامه\\
...
2 100
    ...
    \latin
    \bibitem{learncpp-commandlineargs} Alex. 2008. ``Command Line Arguments'' In \textit{LearnCpp.com}. Retrieved from \url{https://www.learncpp.com/cpp-tutorial/713-command-line-arguments/}.
    \bibitem{wiki-diff} Wikipedia contributors. 2019. ``Diff'' In \textit{Wikipedia, The Free Encyclopedia}. Retrieved from \url{https://en.wikipedia.org/wiki/Diff}.
    \end{thebibliography}
